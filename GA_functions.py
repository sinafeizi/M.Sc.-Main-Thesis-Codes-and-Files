...
def run_evolution(
    ...
    new_file11 = open("results_GA.txt", "a")
    ...
    for i in range(generation_limit):
        ...
        if (round(time() - start_time, 3)) > time_limit:
            print("cost of best: ", cost_func(population[0], slp))
            print("cost of worst: ", cost_func(population[-1], slp))
            break
        best_cost = cost_func(population[0], list_of_all_pairings)
        next_generation = population[0:number_of_elites]
        for j in range(int((len(population) - number_of_elites)/2)):
        ...
        for j24 in range(5):
            population.pop()
        new_small_population = build_population(list_of_all_pairings, list_of_all_flights, 5)
        for j25 in range(len(new_small_population)):
            population.append(new_small_population[j25])
...
run_evolution(slp, to_cover_flights_list, 0.1, 50, 15, 100000, time(), 3600, 2, 2)
