...
def value(a_pairing):
    return a_pairing[-1]/(len(a_pairing) - 1)
...
def run_aco_old(
    ...
    new_file15 = open("results_ACO_old.txt", "a")
    ...
    for w04 in range(generations_limit):
        ...
        sum_of_all = 0
        for w13 in range(len(list_of_all_pairings)):
            sum_of_all += (value(list_of_all_pairings[w13])* pheromones[w13])
        for w09 in range(len(network)):
            probabilities = []
            for w20 in range(len(network[w09])):
                probabilities.append(0)
            for w10 in range(len(network[w09])):
                if pheromones[w10] != 0:
                    probabilities[w10] = ((beta*(value(list_of_all_pairings[w09])* pheromones[w09]))/sum_of_all)
...
run_aco_old(slp, to_cover_flights_list, 20, 1000, 1, 0.85, time(), 600, 1, 1)
